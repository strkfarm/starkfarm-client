...
export function useERC4626Value(token: TokenInfo | undefined) {
    ...
    const { balance, isLoading: isLoadingBal, isError: isErrorBal } = useERC20Balance(token);

    const { data: underlyingBal, isError: isErrorConvert, isLoading: isLoadingConvert, error: errorBal } = useContractRead({
