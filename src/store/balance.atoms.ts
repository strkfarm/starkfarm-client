...
async function getERC20Balance(token: TokenInfo | undefined, address: string | undefined) {
...
async function getERC4626Balance(token: TokenInfo | undefined, address: string | undefined) {
    console.log('balData isERC4626', token?.token);
  if (!token) return returnEmptyBal();
  if (!address) return returnEmptyBal();

  let bal = await getERC20Balance(token, address);
  const provider = new RpcProvider({
    nodeUrl: process.env.NEXT_PUBLIC_RPC_URL
  });
  const erc4626Contract = new Contract(ERC4626Abi, token.token, provider);
  const balance = await erc4626Contract.call('convert_to_assets', [uint256.bnToUint256(bal.amount.toString())]);
...
async function getERC721PositionValue(token: NFTInfo | undefined, address: string | undefined) {
  if (!token) return returnEmptyBal();
  if (!address) return returnEmptyBal();
  ...
  return {
    amount: new MyNumber(uint256.uint256ToBN(result[1].estimated_size).toString(), tokenInfo.decimals),
    tokenInfo: tokenInfo
  }
}
...
export function getBalanceAtom(token: TokenInfo | NFTInfo | undefined, enabledAtom: Atom<boolean>) {
  if (token) {
    if (token.hasOwnProperty('isERC4626')) {
