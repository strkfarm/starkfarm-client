...
async function deserialiseTxInfo(key: string, initialValue: TransactionInfo[]) {
    ...
    const txs: TransactionInfo[] = storedValue ? JSON.parse(storedValue) : initialValue;
    txs.forEach(tx => {
        tx.info.amount = new MyNumber(tx.info.amount.bigNumber.toString(), tx.info.amount.decimals);
...
export const transactionsAtom = createAtomWithStorage<TransactionInfo[]>('transactions', [], deserialiseTxInfo);
  
// call this func to add a new tx to the tx history
// initiates a toast notification
export const monitorNewTxAtom = atom(null, async (get, set, tx: TransactionInfo) => {
    console.log('monitorNewTxAtom', tx)
    await initToast(tx, get, set);
})

async function waitForTransaction(tx: TransactionInfo, get: Getter, set: Setter) {
...
export function StrategyTxPropsToMessageWithStrategies(tx: StrategyTxProps, strategies: StrategyInfo[]) {
    const tokenInfo = TOKENS.find(t => standariseAddress(t.token) === standariseAddress(tx.tokenAddr));
    if (!tokenInfo) {
