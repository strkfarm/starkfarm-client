...
export class AutoTokenStrategy extends IStrategy {
    ...
    depositMethods = (amount: MyNumber, address: string, provider: ProviderInterface) => {
        const baseTokenInfo: TokenInfo = TOKENS.find(t => t.name == this.token) as TokenInfo; //
        const zTokenInfo: TokenInfo = TOKENS.find(t => t.name == this.lpTokenName) as TokenInfo;
    ...
    withdrawMethods = (amount: MyNumber, address: string, provider: ProviderInterface) => {
        const frmToken: TokenInfo = TOKENS.find(t => t.token == this.strategyAddress) as TokenInfo;
